...
- **Do not create a new migration to fix a schema change that was introduced by a migration that has not been committed yet.** If the unreleased head migration needs a correction (e.g., a column needs to be added or removed), `alembic downgrade base`, edit the migration file in place, then `alembic upgrade head` again. Creating an extra migration for an in-flight change bloats the chain with noise and obscures the original intent.
...
- **Always run `alembic heads` before generating a new migration.** The result must show exactly one head. Two heads means two migration files share the same `down_revision`, forming a branch — this breaks `alembic upgrade head` (and `downgrade base`) on every environment, including production, until resolved. The most common cause is writing or generating a migration against an outdated branch after a colleague's migration was already merged. Fix it by updating the newer file's `down_revision` to the real current head; do not generate a new migration on top of a diverged chain, as that will ingest the wrong parent and deepen the problem.
- **Verify `down_revision` after every `autogenerate` run.** Confirm the generated file's `down_revision` matches the single line output by `alembic heads`. If it does not match (e.g., the generator ran against a stale DB state), correct the `down_revision` in the file before running `alembic upgrade head`.
...
First, confirm there is exactly one head:

```bash
alembic heads
```

The output must be a single revision ID followed by `(head)`. If you see two or more lines, the chain has diverged — stop and resolve the branching (see *Known Constraints*) before continuing.
...
- The `down_revision` equals the single revision ID returned by `alembic heads` above. If it does not match (e.g., Alembic resolved against a stale DB state), correct it in the file before proceeding.

After saving any corrections, run `alembic heads` once more and confirm only one head exists.

If `autogenerate` produced incorrect or incomplete SQL beyond the `down_revision`, correct it in the migration file before proceeding.
