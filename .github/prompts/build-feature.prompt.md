...
- Do not modify files under `alembic/versions/` to add application logic — migrations are generated via `alembic revision --autogenerate`. Autogenerated SQL is acceptable to inspect and correct.
...
- **Services never call `db.commit()` or `db.rollback()` directly.** Any service method that must commit (i.e. it writes to the DB and the caller should not be able to add further writes to the same transaction) must accept `uow: UnitOfWorkABC` (from `app.core.unit_of_work`) instead of a raw `AsyncSession`, and call `await uow.commit()`. Read-only methods that do not commit continue to accept `db: AsyncSession`. See [ADR-021](../../docs/design/adr/021-unit-of-work-pattern.md) and `docs/guidelines/feature-architecture.md` §2 (`core/unit_of_work.py`).
...
**Transaction boundaries — UoW for writes, raw session for reads (see [ADR-021](../../docs/design/adr/021-unit-of-work-pattern.md)):**

If the service method **writes** to the DB (creates, updates, or deletes) and must commit when all operations are complete, accept `uow: UnitOfWorkABC` and access the session via `uow.session`:

```python
from app.core.unit_of_work import UnitOfWorkABC

async def create_entity(self, data: dict, actor_id: str, uow: UnitOfWorkABC) -> Entity:
    db = uow.session
    entity = await self.repository.add(db, new_entity)          # flushes, does not commit
    await self.foreign_client.credit(db, actor_id, amount)      # flushes, does not commit
    await uow.commit()                                          # single atomic commit
    await db.refresh(entity)
    return entity
```

If the service method is **read-only** (no writes, no commit), accept `db: AsyncSession` directly:

```python
from sqlalchemy.ext.asyncio import AsyncSession

async def list_entities(self, db: AsyncSession, ...) -> tuple[list[Entity], int]:
...
Place the `record()` call **after** the business operation succeeds. If the operation raises, no audit row is written — which accurately reflects the real outcome. The `AuditTrail` instance is injected via `__init__()` and wired in `composition.py` via `get_audit_trail`. When the method uses a `uow`, pass `uow.session` as the `db` argument to `audit_trail.record()`. Check `docs/specs/non-functional/10-logging-observability.md` for the full list of operations that require an audit row.

### Step 6 — `composition.py`: dependency wiring

Update `get_<entity>_service()` if new dependencies (e.g., a second repository or an external client) are required by this feature. If this feature adds a write endpoint, ensure `get_unit_of_work` is also defined in this module's `composition.py`:
...
No changes needed if the existing factory already covers all dependencies.
...
**Write endpoints inject the Unit of Work; read-only endpoints inject the raw session:**

```python
from app.core.unit_of_work import UnitOfWorkABC
from app.core.database import get_async_db
from app.<module>.composition import get_unit_of_work
from sqlalchemy.ext.asyncio import AsyncSession

# ✅ Write endpoint — use UoW
...
async def create_entity(
    ...
    ...
    result = await service.create_entity(data.model_dump(), str(current_user.id), uow)
    ...

# ✅ Read-only endpoint — raw session is sufficient
@router.get("/entities/")
async def list_entities(
    ...
    ...
    items, total = await service.list_entities(db, ...)
    ...
```

`get_unit_of_work` wraps the same FastAPI-managed `AsyncSession` in a `SQLAlchemyUnitOfWork` and is defined in `composition.py` (see Step 6). Do not call `get_async_db` in write handlers — use `get_unit_of_work` instead.
...
If this feature required changes to the ORM model (new columns, constraints, etc.), follow all steps in `add-migration.prompt.md`. Skip this step entirely if no model changes were made.
