...
Cross-cutting infrastructure (config, DB session factory, JWT, logging, event-driven audit trail, message brokers, schedulers, error builders) lives in `app/core/`. Domain-specific background jobs live under their domain (e.g., `app/purchases/jobs/verify_purchases/`) following the Fan-Out Dispatcher + Per-Item Runner pattern; see `docs/guidelines/background-jobs.md` and ADR-016.
