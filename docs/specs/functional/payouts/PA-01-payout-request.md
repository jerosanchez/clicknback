...
**Then** the payout is successfully requested with status `requested` and available balance is deducted
...
**Then** the request is rejected with insufficient balance error
...
6. System rejects the request with error message.
...
6. System rejects the request with policy error.
