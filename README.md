...
- **Test discipline**: a comprehensive three-layer testing pyramid — unit tests (all dependencies mocked), integration tests (real PostgreSQL database, no mocks), and E2E tests (full Docker Compose stack, coming soon) — with full coverage reporting, strict Arrange-Act-Assert structure, and one canonical example per layer ([ADR-007](docs/design/adr/007-layered-testing-strategy.md)).
